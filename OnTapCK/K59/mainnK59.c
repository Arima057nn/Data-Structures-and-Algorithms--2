...
int InCase6(Graph g, int argc, char *argv[])
{
    int edgee = 0, j, i;
    for (i = 3; i < argc; i++)
    {
        for (j = i + 1; j < argc; j++)
        {
            if (hasEdge(g, atoi(argv[i]), atoi(argv[j])) == 1)
            {
                edgee++;
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (ptr != NULL)
    {
        ...
        case 2:
            ...
            {
                printf("%d\t%s\n", jval_i(node->key), getVertexName(g, jval_i(node->key)));
            }
            break;
        case 3:
        ...
        case 4:
            ...
            break;
        case 5:
            printf("%d\n", outdegree(g, atoi(argv[3])));
            list = outgoingVertices(g, atoi(argv[3]));
            dll_traverse(p, list)
            {
                printf("%d %s %g\n", jval_i(p->val), getVertexName(g, jval_i(p->val)), getEdgeValue(g, atoi(argv[3]), jval_i(p->val)));
            }
            break;
        case 6:
            printf("%d\n", argc - 3);
            for (i = 3; i < argc; i++)
            {
                printf("%d %s\n", atoi(argv[i]), getVertexName(g, atoi(argv[i])));
            }
            printf("%d\n", InCase6(g, argc, argv));
            for (i = 3; i < argc; i++)
            {
                for (j = i + 1; j < argc; j++)
                {
                    if (hasEdge(g, atoi(argv[i]), atoi(argv[j])) == 1)
                    {
                        printf("%d %d %g\n", atoi(argv[i]), atoi(argv[j]), getEdgeValue(g, atoi(argv[i]), atoi(argv[j])));
                    }
                }
            }
            break;
